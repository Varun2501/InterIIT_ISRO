...
f = pd.read_csv('Astrosat.csv') #B catalog
...
df['Closest_Dist'] = np.zeros(len(df))
df['Closest_Name'] = np.zeros(len(df))
df['Closest_Type'] = np.zeros(len(df))
...
for id1, (closest_id2, dist) in enumerate(zip(idx, d2d)):
    ...
    if closest_dist < max_radius:
        if f['Source_Type'][closest_id2] in Required_Types:         
            ...
            df['Closest_Dist'][id1] = closest_dist*3600 
            df['Closest_Name'][id1] = f['SIMBAD_Name'][closest_id2]
            df['Closest_Type'][id1] = f['Source_Type'][closest_id2]
